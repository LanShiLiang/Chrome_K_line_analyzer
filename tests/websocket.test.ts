...
const binanceKline = (overrides: Record<string, unknown> = {}) => ({
  e: 'kline',
  s: 'BTCUSDT',
  k: {
    t: 1710000000000,
...
describe('TradingView WebSocket adapter', () => {
  it('decodes concatenated protocol messages and skips raw and JSON heartbeats', () => {
    ...
    expect(decodeTradingViewMessages(`${framed(first)}${framed('~h~1')}${framed(second)}`)).toEqual(
      [first, second],
    );
    ...
    const frame = framed({
      ...
      p: [
        'cs_1',
        {
          s1: {
            s: [
              { i: 0, v: [0, 1710000000, 10, 12, 9, 11, 100] },
              { i: 1, v: [1, 1710000060, 11, 13, 10, 12, 120] },
            ],
          },
        },
      ],
...
describe('Binance WebSocket adapter', () => {
  it('parses combined and raw kline streams', () => {
    const [combined] = parseBinanceFrame(
      JSON.stringify({ stream: 'btcusdt@kline_1m', data: binanceKline() }),
    );
    ...
    expect(combined.candles[0]).toMatchObject({
      open: 10,
      high: 12,
      low: 9,
      close: 11,
