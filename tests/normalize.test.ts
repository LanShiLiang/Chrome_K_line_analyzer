...
describe('normalizeCandles',()=>{
  it('normalizes arrays and objects, sorts, and keeps the last duplicate',()=>{
    const result=normalizeCandles([{t:2,o:'10',h:'12',l:'9',c:'11',v:'30'},[1,8,11,7,10,20],[1,8,11,7,9,21]]);
  ...
  it('creates market data with identity and quality metadata',()=>{
    ...
    expect(data).toMatchObject({siteId:'binance',symbol:'BTCUSDT',period:'1m',quality:{valid:true,candleCount:80,score:100}});
  });
});

describe('assessQuality',()=>{
  it('reports insufficient data and missing volume',()=>{
    const candles=Array.from({length:20},(_,i)=>({timestamp:i+1,open:1,high:2,low:0,close:1,volume:0}));
    const quality=assessQuality(candles);
    expect(quality.valid).toBe(false);
    expect(quality.warnings).toEqual(expect.arrayContaining([expect.stringContaining('数据不足'),expect.stringContaining('成交量缺失')]));
  });

  it('caps quality at 100',()=>{
    const candles=Array.from({length:200},(_,i)=>({timestamp:i+1,open:1,high:2,low:0,close:1,volume:1}));
