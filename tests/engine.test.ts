...
const market=(candles:Candle[]):MarketData=>({id:'m',siteId:'test',candles,source:{url:'wss://test/kline',adapterId:'test',capturedAt:0},quality:assessQuality(candles)});
const base=Array.from({length:90},(_,i):Candle=>({timestamp:(i+1)*60000,open:100,high:102,low:98,close:100+(i%3),volume:100}));

describe('analyzeMarket',()=>{
  it('detects a volume-confirmed breakout',()=>{
    const result=analyzeMarket(market([...base,{timestamp:91*60000,open:102,high:110,low:101,close:109,volume:300}]),DEFAULT_CONFIG);
    expect(result).toMatchObject({stage:'MARKUP',signal:{action:'BUY',price:109},volumeSummary:{label:'SPIKE'}});
    expect(result.signal.reasonCodes).toContain('B003');
  });

  it('detects a volume-confirmed breakdown',()=>{
    const result=analyzeMarket(market([...base,{timestamp:91*60000,open:98,high:99,low:88,close:89,volume:300}]),DEFAULT_CONFIG);
  ...
  it('detects a spring that recovers above prior support',()=>{
    const result=analyzeMarket(market([...base,{timestamp:91*60000,open:99,high:101,low:95,close:99,volume:100}]),DEFAULT_CONFIG);
  ...
  it('forces HOLD when data is insufficient even if the shape is a breakout',()=>{
    ...
    const result=analyzeMarket(market([...short,{timestamp:21*60000,open:102,high:110,low:101,close:109,volume:300}]),DEFAULT_CONFIG);
  ...
  it('degrades safely when no data exists',()=>{
    ...
    expect(result).toMatchObject({stage:'UNKNOWN',signal:{action:'HOLD',confidence:0},keyLevels:{support:0,resistance:0}});
  });
});
