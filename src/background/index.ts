...
const session = (tabId: number) => {
  if (!sessions.has(tabId)) sessions.set(tabId, { candidates: [] });
...
chrome.runtime.onMessage.addListener((message: ExtensionMessage, sender, sendResponse) => {
  ...
  if (message.type === 'OPEN_DRAWER' && tabId !== undefined) {
    chrome.tabs.sendMessage(tabId, { ...message, type: 'START_SELECTION' });
  ...
  if (message.type === 'MARKET_DATA_CANDIDATES')
    current.candidates = message.payload as RawMarketPayload[];
  if (message.type === 'GET_STATE') {
    sendResponse({ ok: true, data: current });
    return;
  }
  if (message.type === 'RUN_ANALYSIS') {
    // 所有分析都从当前标签页候选中选取，避免跨标签页串数据。
    const requested = message.payload as { candidateId?: string; config?: UserConfig };
    const candidate =
      current.candidates.find((c) => c.id === requested.candidateId) ?? current.candidates[0];
    if (!candidate) {
      sendResponse({
        ok: false,
        error: {
          code: 'E_MARKET_DATA_NOT_FOUND',
          message: '未捕获到行情数据，请刷新页面或切换周期',
          recoverable: true,
        },
      });
      return;
