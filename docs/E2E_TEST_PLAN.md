...
| 自动化范围   | 3 条确定性 E2E                                      |
| 真实环境范围 | Agent 执行 TradingView 冒烟测试                     |
...
2. **Agent 真实站点冒烟**：在真实 TradingView 页面验证当前协议兼容性、Side Panel 行为和行情一致性，输出可人工复核的报告。
...
| L3   | TradingView 真实站点冒烟                 | Agent 控制真实 Chrome            | PR 否，发布前 P0/P1 是 |
