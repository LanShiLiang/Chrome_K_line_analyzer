...
| 数据存储 | Chrome Storage 配置；行情、选区截图和识别结果按 Tab 内存保存             | 避免持久保存原始响应、图像和敏感数据                       |
...
- 用户可配置 30 分钟、1 小时、4 小时、日、周、月周期以及 5–1000 根分析数量；输入框允许清空并在本地显示校验提示，只有点击“开始分析”才发送请求。默认 200 根。
- Tab 导航会清理旧候选和框选；异步分析完成前如果页面已切换，本次结果会被拒绝。
- 框选后由 Service Worker 通过 `activeTab` 截取选区，在本地识别红绿 K 线序列并与标准化行情匹配起止日期；被动批次允许使用图表几何映射降级，无法可靠匹配时返回可见错误。
