...
本文档定义插件如何在用户访问 TradingView 时识别页面内行情连接、采集候选 WebSocket 消息、解析 K 线与成交量数据并标准化为 OHLCV。产品不兼容 Binance 等其他行情网站。

## 2. 设计目标

| 目标         | 说明                                      |
| ------------ | ----------------------------------------- |
| 真实数据优先 | 优先使用页面真实 WebSocket 行情消息       |
| 适配器隔离   | TradingView 协议细节封装在 Adapter 内     |
| 降级可用     | 接口捕获失败时可提示或 DOM 辅助提取       |
| 不越权       | 不读取用户账号、Cookie、Token，不绕过权限 |
| 可测试       | TradingView 适配器使用脱敏帧样本回归测试  |

## 3. 数据采集总体流程

```mermaid
flowchart TD
    Page["TradingView 页面"] --> Hook["Inject Script Hook WebSocket"]
    Hook --> Capture["捕获响应摘要"]
    Capture --> Filter["URL/Content-Type/字段特征过滤"]
    Filter --> Candidate["候选行情数据"]
    Candidate --> Adapter["TradingView 适配器解析"]
...
| 调试诊断     | 记录请求是否发生                                     |
| 协议诊断     | 辅助识别 TradingView 行情连接与协议变化              |
...
T5 实现接口特征过滤和候选数据去重
T6 实现 TradingView SiteAdapter
T7 实现 OHLCV 字段映射和质量校验
T8 实现 DOM 降级识别
T9 建立 fixtures 样本目录和适配器单测
T10 编写 TradingView E2E 验证用例
```
