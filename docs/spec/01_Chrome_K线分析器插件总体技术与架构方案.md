...
项目开发一个面向多个行情网站的 Chrome 插件，当前支持 TradingView、Binance 和同花顺。插件通过 Content Script、Inject Script、Background Service Worker 和侧边抽屉面板协作，按站点选择 WebSocket 解析或匿名行情接口主动请求，将数据统一标准化为 OHLCV，再执行维科夫量价策略分析并渲染同一批 K 线与成交量。
...
| 不承诺未适配站点可用   | 当前仅保证 TradingView、Binance 和同花顺；新增站点需实现并验证独立适配器 |
...
| 支持多站点行情采集   | TradingView 解析页面 WebSocket；Binance、同花顺主动请求匿名行情接口 |
| 支持 OHLCV 标准化    | 将各站点行情数据统一为 Candle 数据模型                              |
...
| F002 | 页面识别 | 识别 TradingView、Binance 和同花顺  | P0     |
| F003 | K 线框选 | 在页面上拖拽选择分析区域            | P0     |
| F004 | 行情采集 | 按站点解析 WebSocket 或主动请求行情 | P0     |
| F005 | 数据解析 | 将各站点原始数据统一转为 OHLCV      | P0     |
| F006 | 策略分析 | 基于维科夫量价规则输出信号          | P0     |
| F007 | 结果展示 | 侧边抽屉展示信号、解释和风险        | P0     |
| F008 | 参数配置 | 支持分析周期和分析 K 线数量配置     | P1     |
...
sequenceDiagram
    ...
    INJ-->>CS: TradingView WebSocket 候选
    CS->>BG: MARKET_RESPONSE_CAPTURED
    BG->>AD: 按站点采集并解析行情
    AD-->>BG: 标准 OHLCV 数据
    BG->>EN: 执行维科夫量价分析
    EN-->>BG: 分析结果
...
| 方案           | 优势                     | 劣势                           | 使用策略              |
| -------------- | ------------------------ | ------------------------------ | --------------------- |
| WebSocket Hook | 能读取页面真实推送       | 需要 MAIN world Content Script | TradingView 主方案    |
| 匿名行情请求   | 可按用户数量直接获取     | 受站点限流和协议变化影响       | Binance、同花顺主方案 |
| 已捕获候选回退 | 主动请求失败时仍可能可用 | 依赖页面是否产生可解析行情     | 降级方案              |

推荐组合：`站点识别 + 独立适配器 + 主动请求/WebSocket Hook + 已捕获候选回退`。
...
| Page Detector   | 判断当前页面属于哪个支持站点 | URL、Host                   | MarketSite       |
| Selection Tool  | 页面框选交互                 | 鼠标事件、视口信息          | SelectionRange   |
| Network Capture | 捕获行情接口候选数据         | WebSocket 消息              | RawMarketPayload |
| Site Adapter    | 构造请求或解析站点协议       | URL、配置、RawMarketPayload | MarketData       |
| Data Normalizer | 统一 OHLCV                   | 各站点行情                  | Candle[]         |
...
| 风险编号 | 风险                  | 影响                | 概率 | 应对措施                        |
| -------- | --------------------- | ------------------- | ---- | ------------------------------- |
| R001     | 行情站点协议变更      | 对应站点采集失败    | 高   | 独立适配器隔离，脱敏样本回归    |
...
| 阶段               | 周期 | 交付物                                         |
| ------------------ | ---: | ---------------------------------------------- |
| 需求确认与技术预研 | 1 周 | 支持站点范围、数据样本、技术验证               |
| 插件基础框架       | 1 周 | Manifest、Vite、React、基础通信                |
| 行情采集与适配器   | 2 周 | WebSocket Hook、主动请求、适配器、OHLCV 标准化 |
...
| 数据采集   | TradingView、Binance、同花顺分别完成目标页面冒烟验证 |
...
T3 实现多站点页面识别、站点注册表和域名配置
T4 实现 K 线框选遮罩和 SelectionRange 计算
T5 实现 WebSocket Hook 与候选行情响应收集
T6 实现 TradingView、Binance、同花顺适配器和 OHLCV 标准化
