...
| 指标 | 计算方式 | 用途 |
|---|---|---|
| 成交量均线 | `SMA(volume, volumeMaPeriod)` | 判断放量/缩量 |
| 放量 | `volume > ma * volumeSpikeRatio` | 突破、派发、恐慌下跌识别 |
| 缩量 | `volume < ma * lowVolumeRatio` | 洗盘、回踩、供应枯竭识别 |
...
| 风险 | 触发条件 | 输出 |
|---|---|---|
| 数据不足 | K 线数量小于 `minCandles` | 不输出强买卖点 |
...
4. 不同市场和周期的阈值需要调参，不应使用单一参数覆盖所有品种。
