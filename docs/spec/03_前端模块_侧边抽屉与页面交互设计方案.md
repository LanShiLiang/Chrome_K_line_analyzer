...
| 适配 TradingView | 页面识别和 UI 展示与协议采集解耦    |
...
stateDiagram-v2
    [*] --> Unsupported
    Unsupported --> Ready: 识别到 TradingView
...
| `rangeLookback`     |     60 | 支撑阻力识别窗口  |
| `breakoutThreshold` |   0.02 | 突破阈值          |
| `volumeSpikeRatio`  |    1.8 | 放量倍数          |
| `lowVolumeRatio`    |    0.6 | 缩量倍数          |
| `minCandles`        |     80 | 最小分析 K 线数量 |
| `maxHistoryItems`   |    100 | 最大历史记录数    |
| `debugMode`         |  false | 调试模式          |

全部参数统一在 Drawer 底部的“策略参数”模块中配置。参数变化后立即更新当前分析状态并写入 `chrome.storage.local`；已有分析结果时同步重新计算，不再提供独立 Options 页面或手动保存按钮。

## 9. 异常状态设计

| 状态                | 用户文案               | 可操作项           |
| ------------------- | ---------------------- | ------------------ |
| 非 TradingView 页面 | 当前仅支持 TradingView | 打开 TradingView   |
| 未框选              | 请先框选 K 线区域      | 开始框选           |
| 未捕获数据          | 未识别到行情接口       | 刷新页面、切换周期 |
| 数据不完整          | 当前行情数据字段不完整 | 重试、提交诊断     |
| 分析失败            | 策略计算失败           | 重试、复制错误     |
