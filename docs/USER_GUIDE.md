...
4. Optionally select a chart area. The rectangle records only viewport coordinates and does not read pixels.
5. Select **Start Analysis**.
6. Review the localized strategy result, stage, confidence, evidence, warnings, and chart.

## Settings

Binance and Tonghuashun allow daily, weekly, or monthly periods and a 20–1000 candle analysis window. One candle-count setting drives both calculation and chart rendering, and valid changes are saved locally.
...
Market data and analysis results stay in the browser and are not uploaded to a developer server. Active public market requests go directly to Binance or Tonghuashun without cookies or login credentials. See the bilingual [Privacy Policy](../PRIVACY.md).
...
- If too few candles are available, lower the requested candle count within the permitted range or choose a page/period with more data.
